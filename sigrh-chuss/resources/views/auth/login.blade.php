...
<body class="font-sans antialiased text-chuss-dark min-h-screen flex items-center justify-center relative overflow-hidden bg-[#faf9f6]">
    ...
    <div class="relative z-10 w-full max-w-[1100px] mx-4 my-4 md:my-8 min-h-fit md:min-h-[650px] flex flex-col md:flex-row bg-white/70 backdrop-blur-3xl rounded-3xl md:rounded-[2.5rem] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.15)] overflow-hidden border border-white/80 transform transition-all duration-500 hover:shadow-[0_40px_70px_-15px_rgba(0,0,0,0.2)]">
        
        <!-- LEFT SIDE (Top on Mobile): Monumental Branding Inside Card -->
        <div class="w-full md:w-5/12 animated-bg relative flex flex-col justify-center items-center py-8 px-4 md:p-14 overflow-hidden shadow-[inset_-20px_0_50px_rgba(0,0,0,0.1)] rounded-b-[3rem] md:rounded-b-none md:rounded-r-[6rem] lg:rounded-r-[12rem] z-20">
            ...
            <div class="relative z-10 flex flex-col items-center text-center my-auto w-full">
                <!-- Suppression du logo "collé" demandée -->
                
                <h1 class="text-3xl md:text-4xl lg:text-5xl font-black text-white leading-tight tracking-tight mb-2 md:mb-4 text-glow">
                    SIGRH <br>
                ...
                <div class="h-1 w-12 bg-chuss-amber rounded-full mb-4 md:mb-6 mt-1 md:mt-2"></div>
                
                <p class="text-sm md:text-lg text-white/90 font-bold leading-relaxed max-w-[280px] md:max-w-xs">
                    Centre Hospitalier Universitaire Sourô Sanou
                </p>
                <p class="text-xs md:text-sm text-white/70 mt-2">
                    Système de Gestion de la Restauration
                </p>
            </div>
        </div>

        <!-- RIGHT SIDE (Bottom on Mobile): Form Inside Card -->
        <div class="w-full md:w-7/12 flex flex-col justify-center items-center p-6 sm:p-8 md:p-12 lg:p-16 relative z-10 bg-white/50 overflow-hidden">
            
            <!-- Local Background Watermark for the form side -->
            <div class="absolute inset-0 z-0 pointer-events-none flex items-center justify-center opacity-10 overflow-hidden hidden md:flex">
            ...
            <div class="w-full max-w-md relative z-10">
                ...
                <div class="mb-6 md:mb-10 text-center md:text-left">
                    <h2 class="text-2xl md:text-3xl font-extrabold text-gray-900 mb-1 md:mb-2 tracking-tight">Bienvenue</h2>
                    <p class="text-sm md:text-base text-gray-500">Connectez-vous à votre espace sécurisé.</p>
                </div>
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-4 md:space-y-6">
                    ...
                    <!-- Identifiant -->
                    <div class="space-y-1.5 md:space-y-2 group">
                        ...
                        <div class="relative">
                            ...
                            <input id="email" type="text" name="email" value="{{ old('email') }}" required autofocus autocomplete="username" 
                                class="input-chic pl-11 block w-full rounded-xl md:rounded-2xl text-gray-900 font-medium px-4 py-3 md:py-3.5 text-sm md:text-base outline-none"
                                placeholder="Entrez votre identifiant" />
                        </div>
                        <x-input-error :messages="$errors->get('email')" class="mt-1 md:mt-2 text-xs md:text-sm text-red-500 font-medium" />
                    </div>

                    <!-- Mot de passe -->
                    <div class="space-y-1.5 md:space-y-2 group pt-1">
                        <div class="flex items-center justify-between ml-1">
                            <label for="password" class="block text-xs font-bold text-gray-600 tracking-wider uppercase">Mot de passe</label>
                            @if (Route::has('password.request'))
                                <a href="{{ route('password.request') }}" class="text-[10px] md:text-xs font-bold text-chuss-green hover:text-chuss-amber transition-colors">
                                    Oublié ?
                                </a>
                            @endif
                        </div>
                        <div class="relative">
                            ...
                            <input id="password" type="password" name="password" required autocomplete="current-password"
                                class="input-chic pl-11 block w-full rounded-xl md:rounded-2xl text-gray-900 font-bold px-4 py-3 md:py-3.5 text-sm md:text-base tracking-widest outline-none"
                                placeholder="••••••••" />
                        </div>
                        <x-input-error :messages="$errors->get('password')" class="mt-1 md:mt-2 text-xs md:text-sm text-red-500 font-medium" />
                    </div>

                    <!-- Options -->
                    <div class="flex items-center pt-1 md:pt-2 ml-1">
                        <div class="relative flex items-start">
                            ...
                            <div class="ml-2.5 text-sm">
                                <label for="remember_me" class="font-semibold text-gray-500 text-xs md:text-sm cursor-pointer select-none hover:text-gray-800 transition-colors">Se souvenir de moi</label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="pt-4 md:pt-6">
                        <button type="submit" class="group relative w-full flex justify-center items-center py-3.5 md:py-4 px-4 rounded-xl md:rounded-2xl overflow-hidden font-bold text-white bg-gradient-to-r from-[#0f3f2a] to-[#1a6a44] hover:from-[#1a6a44] hover:to-[#0f3f2a] transition-all duration-300 transform hover:-translate-y-1 shadow-[0_12px_25px_-8px_rgba(15,63,42,0.5)] focus:outline-none focus:ring-4 focus:ring-[#0f3f2a]/30">
                            ...
                            <span class="relative text-sm md:text-base tracking-wide">Se connecter</span>
                ...
                <p class="text-center text-[10px] text-gray-400 mt-8 md:mt-12 font-bold tracking-widest uppercase">
